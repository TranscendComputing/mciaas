// This is the main package for the mciaas application.
package main

import (
	"flag"
	"fmt"
	"github.com/TranscendComputing/mciaas/restAPI"
	"github.com/TranscendComputing/mciaas/store"
	"github.com/ant0ine/go-json-rest"
	"net/http"
	"path/filepath"
)

type ApplicationContext struct {
	listenPort  int
	storeType   string
	rootPath    string
	applianceId string
	capsuleId   string
	peers       string
}

func (ctx *ApplicationContext) initialize() {
	const (
		defaultPort        = 8080
		portUsage          = "port on which to listen for HTTP(S) requests."
		defaultRoot        = "/tmp"
		rootUsage          = "root directory in which to run all Packer operations."
		defaultApplianceId = ""
		applianceIdUsage   = "appliance id for etcd lookups."
		defaultCapsuleId   = ""
		capsuleIdUsage     = "capsule id for etcd lookups."
		defaultStoreType   = "memory"
		storeTypeUsage     = "storage type to use for templates (memory|etcd)."
		defaultPeers       = "127.0.0.1:4001"
		peersUsage         = "etcd peers, comma separated as in ipaddr:port[,ipaddr:port[...]]"
	)
	flag.IntVar(&ctx.listenPort, "port", defaultPort, portUsage)
	flag.IntVar(&ctx.listenPort, "p", defaultPort, portUsage+" (shorthand)")
	flag.StringVar(&ctx.rootPath, "rootpath", defaultRoot, rootUsage)
	flag.StringVar(&ctx.rootPath, "r", defaultRoot, rootUsage+" (shorthand)")
	flag.StringVar(&ctx.applianceId, "applianceid", defaultApplianceId, applianceIdUsage)
	flag.StringVar(&ctx.applianceId, "a", defaultApplianceId, applianceIdUsage+" (shorthand)")
	flag.StringVar(&ctx.capsuleId, "capsuleid", defaultCapsuleId, capsuleIdUsage)
	flag.StringVar(&ctx.capsuleId, "c", defaultCapsuleId, capsuleIdUsage+" (shorthand)")
	flag.StringVar(&ctx.peers, "peers", defaultPeers, peersUsage)
	flag.StringVar(&ctx.storeType, "storage", defaultStoreType, storeTypeUsage)
	flag.StringVar(&ctx.storeType, "s", defaultStoreType, storeTypeUsage+" (shorthand)")

	flag.Parse()
}

func (ctx *ApplicationContext) realMain() int {
	genericTemplates := JSONTemplates{filepath.Join(ctx.rootPath, "templates")}
	packerTemplates := restAPI.PackerRestAPI{}

	var storage store.Store
	switch ctx.storeType {
	case "etcd":
		storage = &store.EtcdStore{
			Peers:   ctx.peers,
			Debug:   true,
			KeyBase: "/mciaas",
		}
	default:
		storage = &store.MapStore{}
	}

	packerTemplates.SetStorage(&storage)
	images := restAPI.ImageAPI{}
	images.SetStorage(storage)
	images.SetRootPath(ctx.rootPath)

	handler := rest.ResourceHandler{
		EnableRelaxedContentType: true,
	}
	handler.SetRoutes(
		rest.RouteObjectMethod("GET", "/templates", &genericTemplates, "ListTemplateTypes"),
		rest.RouteObjectMethod("GET", "/templates/builders", &genericTemplates, "ListBuilderTemplates"),
		rest.RouteObjectMethod("GET", "/templates/builders/:type", &genericTemplates, "GetBuilderTemplate"),
		rest.RouteObjectMethod("GET", "/templates/provisioners", &genericTemplates, "ListProvisionerTemplates"),
		rest.RouteObjectMethod("GET", "/templates/provisioners/:type", &genericTemplates, "GetProvisionerTemplate"),
		rest.RouteObjectMethod("GET", "/templates/postprocessors", &genericTemplates, "ListPostprocessorTemplates"),
		rest.RouteObjectMethod("GET", "/templates/postprocessors/:type", &genericTemplates, "GetPostprocessorTemplate"),
		rest.RouteObjectMethod("DELETE", "/packer/:user/:docId", &packerTemplates, "Delete"),
		rest.RouteObjectMethod("GET", "/packer/:user/:docId", &packerTemplates, "Get"),
		rest.RouteObjectMethod("POST", "/packer/:user/:docId", &packerTemplates, "Post"),
		rest.RouteObjectMethod("PUT", "/packer/:user", &packerTemplates, "Put"),
		rest.RouteObjectMethod("DELETE", "/image/:user/:docId", &images, "Delete"),
		rest.RouteObjectMethod("GET", "/image/:user/:docId", &images, "Get"),
		rest.RouteObjectMethod("POST", "/image/:user/:docId", &images, "Post"),
		rest.RouteObjectMethod("PUT", "/image/:user/:docId", &images, "Put"),
	)
	http.ListenAndServe(fmt.Sprintf(":%v", ctx.listenPort), &handler)

	return 0
}
